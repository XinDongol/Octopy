...
def main():
    ...
    # ---------------------- Start --------------------------
    # how many rounds you want to run
    for round_idx in range(config.opt.num_rounds):
        ...
        for gpu_launcher in GPU_Containers:
            # gpu_launcher.update_done(done)   # update event for each round
            gpu_launcher.update_true_global(
                global_model)   # pull from global model
            gpu_launcher.all_users_opt.update(user_config)
        # print('Update user config and true global')

        [user_queue_for_processings.put(i)
         for i in range(int(config.opt.num_users))]
