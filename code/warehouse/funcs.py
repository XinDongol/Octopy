...
def state_dict_tonumpy(state_dict):
    new_state_dict = {}
    for key in state_dict.keys():
        new_state_dict[key] = state_dict[key].numpy() if state_dict[key].device == torch.device(
            'cpu') else state_dict[key].cpu().numpy()
    return new_state_dict
...
def state_dict_inplace_update(old_state_dict, new_state_dict):
    for key in old_state_dict.keys():
        old_state_dict[key].data.copy_(
            new_state_dict[key].data.to(old_state_dict[key].device))
